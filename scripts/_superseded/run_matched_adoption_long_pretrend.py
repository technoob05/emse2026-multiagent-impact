...
if __name__ == "__main__":
    ...
    event, summary = summarize_event_contrasts(contrasts, pairs, config)
    pairs.to_csv(output_dir / "matched_adoption_established_pairs.csv", index=False)
    event.to_csv(output_dir / "matched_adoption_established_event.csv", index=False)
    with (output_dir / "matched_adoption_established_summary.json").open(
        "w", encoding="utf-8"
    ) as handle:
