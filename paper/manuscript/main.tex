...
The question is live. A recent position paper argues that agents can already
meet every goal of code review, and that keeping a person in the loop as the
mandatory reviewer ``is a dead end''\ \citep{monperrus2026endofreview}. Our data
speak directly to that: in this corpus the person is not a formality on top of
an agent loop, they are the only part of the loop that visibly closes.
...
from repositories with more than 100 stars, a star-based filter of the kind used
to separate engineered projects from scratch repositories
\citep{munaiah2017curating}. Its event tables contain submitted
...
answered, among triggers that open their own thread. Panel A shows the four
cells. Within one product the link does nothing. Across a product boundary the
answered rate more than doubles. Panel B shows the difference between those two
differences, under repository fixed effects and then with month added, and the
range left by dropping each of the fifteen repositories that contribute most
linked cross-product PRs. The dashed marker is the same contrast when the
link label is shuffled inside each repository. These are links in observed data,
not causes.}
\label{fig:context}
\end{figure}
...
28.3\% of the time with one (Figure~\ref{fig:context}A). The difference between
