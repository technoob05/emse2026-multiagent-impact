...
Where the matched gap actually lives across repositories and product pairs, and which pre-trigger repository descriptors explain the familiar-replier split & \ref{tab:s-heterogeneity} & \pageref{tab:s-heterogeneity} \\
\addlinespace
...
Placebo exposures against the real addressed edge, and what the reply text actually says & \ref{tab:s-specificity} & \pageref{tab:s-specificity} \\
Machine-likeness heuristics on the accounts that write the edges, and the contrast after dropping flagged accounts & \ref{tab:s-actors} & \pageref{tab:s-actors} \\
\addlinespace
...
Which review interactions can carry a reply anchor at all, and the RQ1 owner split under nine burst-window schemes & \ref{tab:s-coverage} & \pageref{tab:s-coverage} \\
...
\subsection{Measurement reach and the burst-window convention}\label{app:coverage}

Two stated conventions bound everything that follows, and
Table~\ref{tab:s-coverage} reports both. Panel A asks how much of the review
record can carry an exact reply anchor at all. Only inline review comments do:
they are 43\% of reviewer-side interaction events and 62\% of triggers, so the
addressed edge is silent about the other 57\% of events and 38\% of triggers by
construction, not by finding. Panel B asks whether the five-minute rapid-burst
window is discoverable from the data. It is not. The trigger-to-next-event gap
has one dominant mode near four minutes and no valley inside any defensible
burst region, so we treat the window as a declared convention and re-run the
RQ1 owner split under nine schemes, including the cuts a change-point rule
selects on its own and per-product cuts. The magnitude of the user-minus-mapped
gap moves between 12.5 and 18.6 points; its sign and its repository-clustered
interval never change.

\input{apx_tables_coverage.tex}
\FloatBarrier
...
\subsection{Where the matched gap lives}\label{app:heterogeneity}

An average over 546 matched pairs can hide a gap that exists in only one
corner of the data, so Table~\ref{tab:s-heterogeneity} opens the average up.
Panel A shows that the pairs are unevenly spread -- one repository holds 21\% of
them and one ordered product pair holds 28\% -- and then re-estimates the gap
inside each product pair large enough to stand alone. The direction is general
by the pre-registered rule: every leave-one-repository-out and
leave-one-product-pair-out refit stays negative, and four of six qualifying
pairs reproduce the sign. The size is not general. The two Codex-authored pairs
carry the average, only one pair has an interval excluding zero on its own, and
the rest sit near zero, which is how the gap should be read. Panel B turns to
the RQ3 familiar-versus-newcomer split and asks what six pre-trigger repository
descriptors explain. They account for 18\% of it; repository fixed effects
absorb most of the rest and leave an interval covering zero. Familiar repliers
and newcomers are largely in different repositories, and the descriptors we can
measure do not say what separates those repositories.

\input{apx_tables_heterogeneity.tex}
\FloatBarrier
...
\subsection{Placebo exposures and what the reply says}\label{app:specificity}
...
Table~\ref{tab:s-specificity}, Panel A, does. Three placebo exposures replace
the real edge in turn: a reply anchored to a different inline comment on the
same PR, a permuted anchor that keeps every reply event but reassigns which
replying PR counts as anchored, and an edge time drawn independently of the
PR's own history. The off-target reply gives the same estimate as the real
edge, and the permuted-anchor null is centred at 16.8 points against an
observed 17.3, so the anchoring itself adds nothing: what the estimate tracks
is that somebody replied inline within the window, which is exactly the coarse
diagnostic in the last row. Only the time-shifted null is centred near zero, so
the landmark rule alone does not manufacture the contrast. Panel B then reads
the reply text. Of 105 user-written edges, 56\% are substantive engineering
prose, 27\% are acknowledgement only, and 17\% route the point onward to a
machine or are GitHub's own button text; a full hand reading of all 105 texts
disagrees with the rule on ten. The contrast survives when only a substantive
reply counts as exposure and is indistinguishable from zero when only a routing
reply does, although both subgroups are small.

\subsection{Automation heuristics on the accounts that write the edges}\label{app:actors}

RQ3 rests on the claim that a person, not a script, writes the addressed edge,
and that claim is an account-type label rather than a verified observation.
Table~\ref{tab:s-actors} stress-tests it. Panel A scores the 80 user accounts
that write the 105 user-written edges on four pre-registered machine-likeness
heuristics -- template repetition, regular inter-comment timing, a missing
diurnal dip, and sustained comment volume -- using 7,141 of their earlier
comments. One account trips the two-heuristic rule and seven trip any single
heuristic, and the highest machine-likeness score in the set is 0.47. Panel B
re-estimates the later-merge contrast after dropping the affected PRs; it moves
up rather than down, and stays interval-positive even under the loosest drop
...
so they cannot detect a person pasting model output; they can only show that
the actor label is not obviously wrong.
