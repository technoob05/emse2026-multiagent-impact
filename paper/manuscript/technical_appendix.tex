...
Participation funnel; fewer than 1\% of trigger PRs carry a different-product exact reply & \ref{tab:s-funnel} & \pageref{tab:s-funnel} \\
...
Tied first timestamps and the fixed priority order behind that split & \ref{tab:s-ties} & \pageref{tab:s-ties} \\
Ordering survives every leave-one-repository-out and leave-one-product-pair-out exclusion & \ref{tab:s-ordering} & \pageref{tab:s-ordering} \\
Within-PR order shuffle against the observed deep transition & \ref{tab:s-deep-transition} & \pageref{tab:s-deep-transition} \\
Escalation and persistence stories rejected by order permutation & \ref{tab:s-falsification} & \pageref{tab:s-falsification} \\
\addlinespace
\multicolumn{3}{@{}l}{\textit{RQ2 -- who bridges the boundary?}} \\
13.4-point matched gap in later visible action across the product boundary & \ref{tab:s-boundary} & \pageref{tab:s-boundary} \\
Seven in ten first user bridges had already reviewed in the repository & \ref{tab:s-history} & \pageref{tab:s-history} \\
\addlinespace
\multicolumn{3}{@{}l}{\textit{RQ3 -- who writes the connection, and does it mark a later state?}} \\
17.3-point addressed-edge contrast, reply windows, repository fixed effects, product-pair drops, specificity control & \ref{tab:s-addressed-edge} & \pageref{tab:s-addressed-edge} \\
Four-state response gradient at the 48-hour landmark & \ref{tab:s-gradient} & \pageref{tab:s-gradient} \\
Automation-to-user route, 12.9 points against automation alone & \ref{tab:s-landmark-route} & \pageref{tab:s-landmark-route} \\
Who writes the edge, stricter edge rules, landmark selection, and the conditional shuffle test at $p=0.007$ & \ref{tab:s-exposure-scope} & \pageref{tab:s-exposure-scope} \\
Whole-population hazard near 1.7, and the edge split by the replier's prior history & \ref{tab:s-extensions} & \pageref{tab:s-extensions} \\
Pre-trigger balance and propensity overlap & \ref{tab:s-balance} & \pageref{tab:s-balance} \\
E-value of 2.27, tipping grid, negative controls, randomisation inference & \ref{tab:s-sensitivity} & \pageref{tab:s-sensitivity} \\
\addlinespace
...
Release layers, units, and event-table coverage & \ref{tab:s-dataset}, \ref{tab:s-coverage} & \pageref{tab:s-dataset} \\
Exact account allowlist and the code-level glossary & \ref{tab:s-identity}, \ref{tab:s-glossary} & \pageref{tab:s-identity} \\
Named estimation specifications and their formulas & \ref{tab:s-specifications} & \pageref{tab:s-specifications} \\
Data-quality, temporal, and leakage gates & \ref{tab:s-quality} & \pageref{tab:s-quality} \\
Column-by-column leakage tags & \ref{tab:s-variables} & \pageref{tab:s-variables} \\
\addlinespace
\multicolumn{3}{@{}l}{\textit{Negative results, external checks, and reproduction}} \\
Structural same-locus overlap, still semantically uncoded & \ref{tab:s-collision} & \pageref{tab:s-collision} \\
External evidence ladder and independent attribution check & \ref{tab:s-external-screen}, \ref{tab:s-external-attribution} & \pageref{tab:s-external-screen} \\
Failed replication gate in SWE-Review-Chat & \ref{tab:s-external-edge} & \pageref{tab:s-external-edge} \\
Kept, relocated, and rejected results & \ref{tab:s-disposition} & \pageref{tab:s-disposition} \\
Ordered run contract and resampling settings & \ref{tab:s-runorder}, \ref{tab:s-resampling} & \pageref{tab:s-runorder} \\
\bottomrule
\end{tabularx}
\endgroup
...
understood or fixed. Public product presence is not private communication.
Temporal succession is not semantic response. Account history is not verified
memory retrieval. Merge is an integration state, not code quality.

Every outcome model in this paper is therefore an observational association.
None of them identifies an intervention. Task difficulty, maintainer intent,
review meaning, and private activity are not measured, and any of them can
select both the public topology and the later state. Section~\ref{app:bounds}
bounds how much unmeasured structure would be needed to remove the main
association; it does not convert that association into an effect. These
boundaries are part of the result, not only limitations. They hold for every
number in this appendix, so later sections add only what is specific to their
own analysis instead of restating them.
...
identities, given in full in Table~\ref{tab:s-identity}. Similar names and
...
\section{Cohorts, denominators, and estimation specifications}\label{app:cohorts}
...
\subsection{Named specifications}\label{app:specifications}
...
Table~\ref{tab:s-specifications} lists each named specification: its outcome,
exposure term, estimator, standard-error type, denominators, and full model
formula. Most formulas are stored with the analysis products themselves, so the
table cannot drift from the fits it describes; the two ownership-route rows and
the two prior-history rows are read back from the analysis code, which writes no
formula column. Reply-window variants at 1, 6, and 24 hours refit the same
formula with that window's exposure term. The table shows what was fitted and on
how many PRs; it does not show that any fit identifies a causal effect
(Section~\ref{app:boundary}). A linear probability model can also predict outside
the unit interval.
...
\subsection{Burst thresholds and the tie rule}\label{app:burst}
...
\subsection{Leave-one-out ordering robustness}\label{app:ordering}
...
Table~\ref{tab:s-landmark-route} gives every route contrast. Both estimates are
...
First, the exposure rule (Section~\ref{app:reply-target}) does not require
another product to write the reply. Of the 128 exposure events on the 109
exposed PRs, 105 are written by
...
The bounds in Section~\ref{app:bounds} all keep the hour-48 cohort fixed. Two
further analyses change the design instead, and Table~\ref{tab:s-extensions}
reports both.
...
\subsection{Measured balance and overlap}\label{app:balance}

Table~\ref{tab:s-balance} reports what the measured controls actually do. It
gives the exposed mean, the unexposed mean, and the standardized mean difference
for all six pre-trigger controls at all four reply windows. No absolute
standardized difference exceeds 0.16. The largest values sit on log pre-trigger
events, trigger age, and pre-trigger decisive reviews. The final block adds the
...
\subsection{Bounding the unmeasured structure}\label{app:bounds}
...
Section~\ref{app:identity} records. A trigger that sits mid-thread therefore
...
point is not a resolved one; this section measures who replies, not what the
reply achieved.
...
\input{apx_tables_collision.tex}
\FloatBarrier

\section{External attribution and construct checks}\label{app:external}

\subsection{Evidence ladder and attribution concordance}\label{app:ladder}
...
\subsection{A failed replication gate}\label{app:failed-gate}
...
\subsection{Validation gates}\label{app:gates}
...
\subsection{Variable dictionary and leakage tags}\label{app:variables}
...
where each value can be observed, not whether it is a good measure of the thing
it is named after.

\input{apx_tables_quality.tex}
\FloatBarrier
...
\input{apx_tables_disposition.tex}
\FloatBarrier
...
\subsection{Uncertainty settings}\label{app:resampling}
...
repository share policy, integrations, and contributors. The draw count is
